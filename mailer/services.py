...
def decrypt_json(cipher_text: str) -> dict: 
    try: 
        ...
        raise MailerConfigError("Saved encrypted credentials could not be decrypted.") from exc 
...
def send_smtp_message( 
    *, 
    ...
    secret = decrypt_json(connection.encrypted_password) 
    password = secret["password"] 
 
    message = EmailMessage() 
 
    if connection.display_name: 
        message["From"] = f"{connection.display_name} <{connection.from_email}>" 
    else: 
        message["From"] = connection.from_email 
 
    message["To"] = recipient_email 
    message["Subject"] = render_message_content(subject, recipient_email) 
 
    if connection.reply_to_email: 
        message["Reply-To"] = connection.reply_to_email 
 
    message.set_content(render_message_content(body, recipient_email)) 
 
    try: 
        if connection.use_ssl: 
            with smtplib.SMTP_SSL(connection.smtp_host, connection.smtp_port) as server: 
                server.login(connection.username, password) 
                server.send_message(message) 
        else: 
            with smtplib.SMTP(connection.smtp_host, connection.smtp_port) as server: 
                if connection.use_tls: 
                    server.starttls() 
                server.login(connection.username, password) 
                server.send_message(message) 
    except Exception as exc: 
        raise MailerConfigError(str(exc)) from exc 
 
    connection.save(update_fields=["updated_at"]) 
    return f"smtp-{timezone.now().timestamp()}" 
...
def process_pending_jobs(*, limit: int = 20, batch_id: int | None = None, pause_seconds: float = 
0.0) -> int: 
    processed = 0 
    queryset = RecipientJob.objects.select_related( 
        "batch", 
        "batch__gmail_connection", 
        "batch__smtp_connection", 
        "batch__user", 
    ) 
 
    queryset = queryset.filter(status=RecipientJob.Status.PENDING).filter( 
        Q(batch__gmail_connection__is_active=True) | 
        Q(batch__smtp_connection__is_active=True) 
    ) 
    ...
    for job in queryset: 
        ...
        try: 
            if batch.smtp_connection: 
                message_id = send_smtp_message( 
                    connection=batch.smtp_connection, 
